...
import { filterByDateRange, getDateRangeBounds } from "../../Admin/utils/dateUtils";
...
const Reports = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const query = user?.id ? `?trainerUserId=${user.id}` : "";
  ...
  const findAssignment = (record) => {
    ...
    const normalized = {
      ...
      userUuid: record.user_id_uuid || record.userUuid || record.uuid,
      ...
      username: record.username || record.userName || record.name || "",
  ...
  const isAssignedToCurrentTrainer = (record) => {
    if (record.trainer_name || record.trainerName) {
      const name = String(record.trainer_name || record.trainerName).toLowerCase();
      if (name === String(user?.username).toLowerCase()) return true;
    }
